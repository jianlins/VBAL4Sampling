...
def spans_to_bio(doc:Doc, anno_types:List[str], abbr:bool=False)->str:
  ...
  bio_text = []
  bio_data={'sentence_id':[],'doc_name':[], 'token':[],'label':[], 'sentence':[]}
  for s,sent in enumerate(doc.sents):
    for i,token in enumerate(sent):
      ...
      bio_data['sentence'].append(str(sent))        
...
Convert a list of documents into sentence level dataframe
The output dataframe has 4 columns: sentence_id	doc_name	token	label
'''
...
#convert word into feature dictionary
...
#convert single sentence into feature(dic) word by word
#return a list of dictionaries(features)
...
class CRFModel(object):
    ...
    def __init__(self, anno_types=[], topNUncertainToken=4):
        self.reset_model()
    ...
    def reset_model(self):
        ''' keep the CRF model parameter for the training in each round of sampling'''
        self.crf = CRF(
            algorithm='lbfgs',
            c1=0.1,
            c2=0.1,
    ...
    def __df2features(self, df:pd.DataFrame):
        ...
        This hidden function will convert input dataframe into feature vector
        '''
        original_idx=[]
        X=[]
        y=[]
        ...
        sorted_X=sorted(zip(original_idx, X), key=lambda x: x[0]) # keep the sentences in order
    ...
    def df2features(self, df:pd.DataFrame):
        # same function as __df2features but can be called outside for testing
        original_idx=[]
        X=[]
        y=[]
        for idx, sdf in df.groupby('sentence_id'): #iterate through group: idx is the certain sentenceID, sdf is the sub df under certain sentence ID
    ...
    def fit(self, df_sampled:pd.DataFrame):
        '''
        input is the dataframe of sampled sentences: sentence_id	doc_name	token	label
        Before this run: _, train_df=convert_docs(docs, anno_types=self.anno_types)
        '''
        X_train, y_train=self.__df2features(df_sampled)
    ...
    def transform_DFsent(self, df:pd.DataFrame):
        '''need to run _, test_df=convert_docs(docs, anno_types=self.anno_types) before this function'''
        X_test, y_test=self.__df2features(df) #extract the origianl label
    ...
    def transform(self, docs:List[Doc]):
        ...
        these are testing documents.
        It will ouput sentence level prediction
        '''
    ...
    def eval_DFsent(self, df:pd.DataFrame):
        logger.debug('Predicting eval docs...')
        y_test, y_pred =self.transform_DFsent(df)
    ...
    def eval(self, docs:List[Doc]):
        ''' given list of test documents, return sentence level performance metrics'''
        logger.debug('Predicting eval docs...')
    ...
    def eval_scores_DFsent(self, df:pd.DataFrame):
        results_df, FP_ids, FN_ids, micro_precision, micro_recall, micro_f1=self.eval_DFsent(df)
    ...
    def bootstrap_eval_DFsent(self, df:pd.DataFrame, times=200):
        logger.debug('Predicting eval sents...')
        y_test, y_pred =self.transform_DFsent(df)
    ...
    def estimate_certainties(self, df_remained:pd.DataFrame): #remaining sentences certainties
        '''
        Need to run _, df=convert_docs(docs, anno_types=self.anno_types) before this step
        '''
        X, y=self.__df2features(df_remained)
        yp=self.crf.predict_marginals(X)
        sdf=df_remained.drop_duplicates('sentence_id')[['doc_name', 'sentence_id']].reset_index(drop=True)
        sentence_certainties=self.calculate_certainties(yp) #certainties for each sentence
        sdf['certainty']=sentence_certainties # now it has 3 columns: doc_name, sentence_id, certainty
        return sdf
